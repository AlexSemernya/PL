      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1 bg-icon-bg rounded-lg p-0.5">
          <button
            ...
            className={`w-8 h-7 flex items-center justify-center rounded-md transition-colors ${mode === 'week' ? 'bg-card shadow-sm' : ''}`}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={mode === 'week' ? 'var(--color-text)' : 'var(--color-text-muted)'} strokeWidth="2">
          ...
          <button
            ...
            className={`w-8 h-7 flex items-center justify-center rounded-md transition-colors ${mode === 'month' ? 'bg-card shadow-sm' : ''}`}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={mode === 'month' ? 'var(--color-text)' : 'var(--color-text-muted)'} strokeWidth="2">
